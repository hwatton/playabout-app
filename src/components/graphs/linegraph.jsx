...
const LineGraph = (props) => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < newData.length; i++) {
      lineData.push({ x: i + 0.5, y: newData[i].cv });
    }
    ...
    update
      ...
      .style("stroke", (d) => {
        return d3.interpolateCool(d.lineColour);
      })
      .style("fill", "none");

    /* axes */

    d3.selectAll(".axisgroupLine").remove();

    const axisLeft = svgC
      .append("g")
      .attr("class", "axisgroupLine")
      .attr("transform", "translate(20,10)");
    axisLeft.call(d3.axisLeft(yAxisScale).ticks(5));

    const axisBottom = svgC
      .append("g")
      .attr("class", "axisgroupLine")
      .attr("transform", "translate(0, 190)");

    axisBottom.call(d3.axisBottom(xScale).ticks(0));
  }, [props]);
